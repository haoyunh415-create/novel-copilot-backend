...
(() => {
  ...
  // CSP 兜底：部分小说网站阻止外部脚本加载，此时用 GM_xmlhttpRequest 内联注入
  function _loadVisNetwork() {
    ...
    s.onerror = function() {
      // CSP 阻止外部脚本 → 用 GM_xmlhttpRequest 绕过 CSP 拉取后内联注入
  ...
  async function consumeSSE(response, callbacks) {
    // 兼容非流式响应（GM_xmlhttpRequest 无 ReadableStream 时）
  ...
  async function redeemAccount() {
    ...
    try {
      var API = await getAPI();
      var resp = await safeFetch(API + "/api/redeem", {
        method: "POST",
