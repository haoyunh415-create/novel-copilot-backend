(() => {
  ...
  function getChapterTitle() {
    ...
    if (isQQBookSite()) {
      const qd = getQQNuxtData();
      const qt = qd && (qd.chapterTitle || qd.chapterName);
  ...
  function getChapterText() {
    ...
    if (isQQBookSite()) {
      ...
      const qqd = getQQNuxtData();
      const qqCc = qqd && qqd.currentContent;
      if (qqCc && qqCc.content) {
        let qqText = stripHtmlTags(qqCc.content);
        qqText = (qqText || "").split("\n").map((l) => l.trim()).filter((l) => l.length > 3).slice(0, 150).join("\n");
        if (qqText.length >= 80) {
          _cachedText = qqText;
          _cachedTextUrl = location.href;
          return qqText;
    ...
    for (var i = 0; i < bestText.length; i++) {
      ...
      if ((cp >= 0xE000 && cp <= 0xF8FF) || (cp >= 0xF0000 && cp <= 0xFFFFD)) {
        puaCount++;
        if (cp > 0xFFFF) i++;  // 补充私用区占 2 个 UTF-16 码元
      }
    }
    var puaRatio = bestText.length > 0 ? puaCount / bestText.length : 0;
    if (puaRatio > 0.15) {
      console.warn("[鉴来助手] 正文疑似字体加密乱码，PUA 占比 " + (puaRatio * 100).toFixed(1) + "%");
      _cachedText = "";
      _cachedTextUrl = location.href;
      return "";
    }
    // 质量检测 2：中文占比太低说明是乱码/混淆，返回空让上层提示用户
  ...
  // QQ阅读 VIP/会员章节正文为加密（encrypt）或字体混淆（fontEncrypt），扩展内无法解密
  function isQQBookLocked() {
    try {
      const qd = getQQNuxtData();
      const cc = qd && qd.currentContent;
  ...
  function getBookTitleRaw() {
    ...
    if (isQQBookSite()) {
      ...
      const qbt = qbMeta ? (qbMeta.getAttribute("content") || "").trim() : "";
      if (qbt) return qbt;
  ...
  function getAuthor() {
    ...
    if (isQQBookSite()) {
      const qaMeta = document.querySelector("meta[property='og:novel:author']");
      const qat = qaMeta ? (qaMeta.getAttribute("content") || "").trim() : "";
      if (qat) return qat;
    }
    const selectors = [
  ...
  function getChapterIndex() {
    ...
    if (isQQBookSite()) {
      const qd = getQQNuxtData();
      if (qd && qd.cid) {
        const ci = parseInt(qd.cid, 10);
        if (ci > 0 && ci < 100000000) return ci;
      }
      const qm = location.pathname.match(/\/book-read\/\d+\/(\d+)\/?/);
      if (qm) {
        const c2 = parseInt(qm[1], 10);
  ...
  function createWindow() {
    ...
    win.innerHTML =
      ...
      '<div class="jl-footer">' +
        ...
        '</div>' +
        '<div class="jl-actions" style="flex-wrap:wrap">' +
  ...
  async function consumeSSE(response, callbacks) {
    // callbacks: { onProgress(stage, elapsed), onSummary(data), onDone(data), onError(message) }
    ...
    try {
      ...
      // 流结束：flush 解码器残留字节，处理 buffer 里最后一个事件（避免 done 丢失）
  ...
  async function runAnalyze() {
    ...
    try {
      ...
      // 重置伏笔回收卡片
      var payoffCard2 = document.getElementById("jl-payoff-card");
      if (payoffCard2) payoffCard2.style.display = "none";
  ...
  function countChapterLinks() {
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      if (globalThis.JLBatchParser.isChapterTitle(title) || globalThis.JLBatchParser.looksLikeChapterHref(href)) n++;
    }
    return n;
  }
  ...
  function showBatchChapterPicker(all) {
    ...
    var catKey = (globalThis.JLBatchParser && globalThis.JLBatchParser.catalogSortKey) || function (c) { return (typeof c.sort_index === "number") ? c.sort_index : c.chapter_index; };
  ...
  // opts: {title, message, confirmText, cancelText}；confirmText 非空为确认框（两按钮），否则纯提示（单按钮）
  // 返回 Promise<boolean>：确认/知道了 → true；取消/关闭 → false
  ...
  // 番茄阅读页书名是 <h1 class="muye-reader-nav-title">（内嵌返回箭头 SVG，非 <a>），
  // 页面里也没有 /page/{book_id} 链接；book_id 实际嵌在 SSR 的 __INITIAL_STATE__.reader.chapterData.bookId 里
  ...
  // 判断 URL 是否跨域（相对当前页 host）：跨域正文（如纵横 read.zongheng.com）需经后台代理
  ...
  // 经后台 service worker 代理抓正文 HTML（content script 受 CORS 限制，后台有 host_permissions 可跨域）
  function fetchTextViaBackground(url) {
    return new Promise(function (resolve, reject) {
      try {
        chrome.runtime.sendMessage({ type: "jl-fetch-text", url: url }, function (resp) {
          if (chrome.runtime.lastError) { reject(new Error(chrome.runtime.lastError.message)); return; }
          // 非 200（付费/锁定页）也要带回正文 HTML，供 isPaywall 判定「付费跳过」而非「抓取失败」
          if (resp && typeof resp.text === "string") resolve(resp.text);
          else reject(new Error((resp && resp.error) || ("HTTP " + (resp && resp.status))));
        });
      } catch (e) { reject(e); }
    });
  }

  async function fetchChapterHtml(source_url) {
    if (isCrossOrigin(source_url)) {
      try { return await fetchTextViaBackground(source_url); } catch (_) {}
    }
  ...
  // 合并单章分析到全书聚合对象（人物/伏笔/名词/关系图节点边均去重）
  ...
  // 把聚合对象转换成 renderResult 需要的结果结构（复用概况/伏笔/关系图渲染）
  function mergedToResult() {
    ...
    // 若 AI 未返回 graph.nodes，回退用「人物」列表合成节点，保证关系图标签不空白
  ...
  // 每章摘要追加为概况面板里的一张卡片：章节名醒目 + 序号徽章 + 摘要完整展示
  function renderBatchChapterList() {
    ...
    // 并行抓取/提交导致各章摘要按「完成顺序」乱序，这里按阅读顺序升序重排（缺序号放末尾）
  ...
  async function runBatchJob(jobData) {
    ...
    // 打开/复用主窗口，结果渲染进概况/伏笔/关系图 tabs（不再另开浮层）
    createWindow();
    ...
    // 2) 并行提交（并发 2）：付费/空章节跳过，成功即并入全书聚合
    var SUBMIT_POOL = 2;
    var si = 0;
    var submitWorkers = [];
